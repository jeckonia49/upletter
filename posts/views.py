...
class PostListView(PaginationMixin,CategoryMixin, TemplateView):
    """
...
class PostDetailView(ItemNextPrevMixin, CategoryMixin, TemplateView):
    ...
    def get_related_posts(self, **kwargs):
        ...
        form the listing
        """
        instance = self.get_post_object(**kwargs)
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['post'] = self.get_post_object(**kwargs)
...
class CategoryPostListView(PaginationMixin,CategoryMixin, TemplateView):
    """
    This inherits from pagination to handle pagination,
    CategoryMixin removes duplication from the code
    TemplateView is a helper class for handling the view logic
